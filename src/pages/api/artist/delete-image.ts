...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    const { bandId, imageId, storagePath } = await request.json();

    if (!bandId || !imageId || !storagePath) {
      return new Response(JSON.stringify({ error: 'Missing bandId, imageId, or storagePath' }), { status: 400 });
    }
    ...
    // Delete image doc
    await artistRef.collection('images').doc(imageId).delete();

    // Recount images — if zero, flip visible false
    const remaining = await artistRef.collection('images').get();
    let visible = artistSnap.data()?.visible ?? false;
    if (remaining.size === 0) {
      await artistRef.update({ visible: false });
      visible = false;
    }

    return new Response(JSON.stringify({ success: true, visible }), { status: 200 });
  } catch (error) {
